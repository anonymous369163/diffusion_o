...
class COMetaModel(pl.LightningModule):
  ...
  def get_collate_fn(self):
    """获取collate函数，子类可以重写此方法来提供自定义的collate函数"""
    return None  # 返回None表示使用默认的collate函数
  ...
  def train_dataloader(self):
    ...
    if collate_fn is not None:
      # 如果有自定义collate函数，使用标准DataLoader
      import torch.utils.data
  ...
  def test_dataloader(self):
    ...
    if collate_fn is not None:
      ...
      test_dataloader = torch.utils.data.DataLoader(
          self.test_dataset, batch_size=batch_size, shuffle=False, collate_fn=collate_fn)
    else:
      # 否则使用GraphDataLoader
